...
def jugar ():
    ...
    attempts = 0
    name = input("May I ask your name? ")  #variable en la que vamos a guardar el nombre del jugador
    ...
    while True:  # bucle que comprueba si hemos acertado el numero o no
        guess = int(input("Guess the secret number (between 1 and 30): ")) #pedimos que se vaya introduciendo un numero entre 1 y 30 y lo comparamos con el numero aleatorio generado
        attempts += 1
        if guess == secret:
            ...
            score_list.append({"attempts": attempts, "date": str(fecha.ctime()), "name": name, "secret number": secret,
                               "wrong_guesses": wrong_guess})  # añadimos un diccionario dentro de nuestra lista
